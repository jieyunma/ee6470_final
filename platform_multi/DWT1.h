...
const int HiD1[8] = {-4, -2, 13, 38, -103, 64, 4, -10};
const int LoD1[8] = {-10, -4, 64, 103, 38, -13, -2, 4};

struct DWT_1 : public sc_module {
    tlm_utils::simple_target_socket<DWT_1> tsock;

	sc_fifo< sc_dt::sc_int<32> > i_spike;
	sc_fifo< sc_dt::sc_int<32> > o_D;
	sc_fifo< sc_dt::sc_int<32> > o_A;

    SC_HAS_PROCESS(DWT_1);

    DWT_1(sc_module_name n):
        sc_module(n), 
        tsock("t_skt") {
        SC_THREAD( conv );

        tsock.register_b_transport(this, &DWT_1::blocking_transport);
    }
...
// cout << "Get: " << spike1 << "," << spike2 << endl;
            if (m >= 8) {
                ...
                for (int n = 0; n < 8; n++) {
                    val_D += spike_buffer[n] * HiD1[n];
                    val_A += spike_buffer[n] * LoD1[n];
                }
                o_D.write(val_D);
                o_A.write(val_A);
    ...
    void blocking_transport(tlm::tlm_generic_payload &payload, sc_core::sc_time &delay) {
        ...
        switch (payload.get_command()) {
            case tlm::TLM_READ_COMMAND:
                ...
                switch (addr) {
                    case DETAIL_ADDR:
                        // cout << "dwt1 socket try read detail: " << endl;
                        detail = o_D.read();
                        cout << "dwt1 socket read detail: " << detail << endl;
                        break;
                    case APPROX_ADDR:
                        // cout << "dwt1 socket try read approx: " << endl;
                        approx = o_A.read();
                        cout << "dwt1 socket read approx: " << approx << endl;
                        break;                        
                    default:
